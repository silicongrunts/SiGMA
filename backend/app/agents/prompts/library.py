...
- keyword: SQL-based case-insensitive substring match across title, description, and content. Use when searching for specific terms, names, or identifiers.
...
- semantic mode: each <match> shows a relevant chunk (preview truncated to 300 chars) with its relevance score and starting line number. The same document may appear with multiple chunks, up to the system's per-document match limit.
...
- file: A file path (absolute, or relative to the project root). Plain-text/code files, HTML, and PDF/Office (.doc(x), .xls(x), .ppt(x))/EPUB documents are accepted; other extensions are rejected. Files outside the project sandbox are readable for ingestion. Source will be the original path (relative to project root when the file is inside the project, absolute otherwise).
...
Output: "Successfully created folder \"{title}\" (ID: {id}) in root", or "... in {parent title} (ID: {parent id})" when parent_id is given."""
...
PROMPT_LIBRARY_GET = """Read a document's fields from the library. Supports paginated content reading with line numbers.

offset is 0-indexed and must be >=0. limit: omit or pass 0 for 200; negative returns the last abs(limit) lines. Output uses `cat -n`-style line numbers (`N\\t<content>`) with the document's real 1-indexed line; whenever the window stops short of the end, a "Showing lines X-Y of Z (N more not shown)" footer is appended.
